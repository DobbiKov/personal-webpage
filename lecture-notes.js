(() => {
  ...
  const contentContainer = document.querySelector(".lect_notes_content-container");
  const tocContainer = document.querySelector(".table-of-contents");
  const reduceMotionQuery = window.matchMedia("(prefers-reduced-motion: reduce)");
  const desktopLayoutQuery = window.matchMedia("(min-width: 981px)");
  ...
  const updateTocPositionMode = () => {
    if (!contentContainer || !tocContainer) {
      return;
    }

    const shouldUseDesktopMode = desktopLayoutQuery.matches && tocList.children.length > 0;
    if (!shouldUseDesktopMode) {
      contentContainer.classList.remove("toc-center-sticky");
      return;
    }

    const stickyStartOffset = contentContainer.getBoundingClientRect().top + window.scrollY - 16;
    const shouldCenter = window.scrollY > stickyStartOffset;
    contentContainer.classList.toggle("toc-center-sticky", shouldCenter);
  };
  ...
  const render = (data) => {
    ...
    data.forEach((section, index) => {
      ...
      const { item, button } = createTocEntry(section.title || `Section ${index + 1}`, () => {
    ...
    requestAnimationFrame(updateTocPositionMode);
